...
function nu(este){
	...
	$("#nu"+i).append($('<li><input data-t="planta" data-i="'+i+'" type="text"><button onclick="su(this)" class="btn btn-success btn-xs"><i class="fa fa-save"></i></button><button data-i="'+i+'" onclick="ca(this)" class="btn btn-danger btn-xs"><i class="fa fa-close"></i></button></li>'));
}
function ca(este){
	var i=$(este).data("i");
		...
		success:function(data){
			if(data.status==200){
				...
				if(s==0){
					...
					$("#nu"+i).append($('<li><a class="name" href="'+URL_BASE+'/planta/'+data.data.id+'">'+data.data.name+'  </a></li>'));
				}
