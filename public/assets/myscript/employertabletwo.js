...
function shiftableEmployer(datestart,dateini,datefin,unid,cant,element,xx,yy){
    ...
    if(compareDate(myDate2,myDate1)==1){
        myDate=myDate2;
        dateini=datestart;
    }
    ...
    //head+="<td style='background: #42b38b; width:75px'>Unidad</td><td><div id='contEncCol' style='overflow:hidden; overflow-y:scroll; background:#80e0be; width:1070px'><table class='tabla' id='encCol'><tr>";
    head+="<td style='background: #42b38b; width:75px;'>Unidad</td><td><div id='contEncCol' style='overflow:hidden; overflow-y:scroll; background:#80e0be; width:"+ancho+"em'><table class='tabla' id='encCol'><tr>";
    var dia="";
    ...
    switch (unid){
        case "Dia":
            ...
            while(compareDate(auxdate,auxdate2)==-1 || compareDate(auxdate,auxdate2)==0) {
                dia+="<tr><td style='height: 23px;width: 75px;font-size: 14px'>"+(auxdate.getMonth()+1)+"-"+auxdate.getDate()+"-"+getDaySpaAbr(auxdate)+"</td></tr>";
