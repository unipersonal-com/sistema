...
<html lang="en" style="background-color: #f5f5f5;">
    ...
    <body>
        <header>
            ...
            <div id="tipocontrato">
                <p> TIPO DE CONTRATO DE:  <b>{{$nombre_tc}} </b></p>
            </div>
        ...
        @foreach ($per_tc as $tc)
            ...
            ?>
            <div class="container">
                <p style="text-align: center; text-transform: capitalize !important; font-size:10px; margin-top: -10px;">Nombre: <b>{{$tc->nombres.' '.$tc->apellido_paterno.' '.$tc->apellido_materno}}</b></p>
                <table class="table__kiri table-bordered">
                    ...
                    <tbody>
                        @foreach ($cadena as $fecha)
                            ...
                            @endif
                            @if ($existe > 0)
                                @foreach ($grupohorario as $permis)
                                    @if ($permis->start == $fecha && $permis->id_persona == $tc->id)
                                    <tr>
                                        <td>{{$permis->start}}</td>
                                        <td>{{DATE::create($permis->start)->format('D')}}</td>
                                        <td>{{$permis->ci}}</td>
                                        <td>{{$permis->title}}</td>
                                        <td>{{$permis->nombre_tiposalida}}</td>
                                        <td>{{$permis->nombre_horario}}</td>
                                        <td>{{$permis->inicio_time}}</td>
                                        <td>{{$permis->fin_evento}}</td>
                                        @php
